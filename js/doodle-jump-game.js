// Doodle Jump Game - Upgraded Version
// 시현이 사이트용 업그레이드 버전
...
class DoodleJumpScene extends Phaser.Scene {
  ...
  setupTouchControls(){
    ...
    this.input.on('pointerdown',(p)=>{
      ...
      if(p.x<GAME_CONFIG.width/2) this.touchLeft=true
  ...
  shoot(){
    ...
    if(this.player.facingDirection==='left'){
      bullet.body.setVelocity(-speed,-200)
    }else{
      bullet.body.setVelocity(speed,-200)
    }
  ...
  updateMovement(){
    ...
    if(this.player.x<0)
      this.player.x=GAME_CONFIG.width

    if(this.player.x>GAME_CONFIG.width)
      this.player.x=0
